...
- [ ] 模块级差异矩阵（现有模块 → 目标架构组件映射表）
- [ ] 隐式状态盘点（现有代码中所有状态变量、状态流转逻辑梳理）
- [ ] 平台相关代码盘点（所有 `#[cfg(target_os)]` 列表 → PAL 抽离清单）
- [ ] 外部依赖盘点（系统调用、文件路径、命令执行点）
- [ ] **交付物**：《现状-目标架构差异矩阵》、《PAL 抽离清单》

#### W-1.2 迁移设计书（3 天）

- [ ] Strangler Pattern 迁移策略（新旧并存方案）
- [ ] 模块拆分顺序与依赖图
- [ ] 数据迁移方案（旧存储 → SQLite）
- [ ] 风险点清单与回滚预案
- [ ] **交付物**：《迁移设计书 v1.0》
...
- [ ] 修复编译告警（`#![deny(warnings)]` 在 CI 启用）
- [ ] 修复明显的 unwrap/panic 风险点
- [ ] 修复明显的路径穿越、命令注入风险
- [ ] 修复跨平台编译失败
- [ ] **交付物**：清洁的现状代码基线

#### W-1.4 三平台 CI 紧急搭建（2 天）

- [ ] GitHub Actions / GitLab CI 矩阵：Linux x64 / Windows x64 / macOS arm64
- [ ] 编译 + 单元测试 + clippy + fmt 检查
- [ ] **交付物**：三平台 CI 流水线

#### W-1.5 ADR 启动（2 天）

- [ ] 建立 `docs/adr/` 目录与模板
- [ ] 补录现有架构决策（ADR-001 ~ ADR-012）
- [ ] **交付物**：ADR 目录与首批 12 条记录
